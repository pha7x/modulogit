...
#-------------------------------------------------------
param([parameter(Mandatory=$true)][ValidateSet("Label", "DataCheck", "Antenna", "PSN", "Current", IgnoreCase=$true)]$Global:OperationPhase)
...
if ($model) {
    ...
    $XMLcfgFileName = Join-Path $((Get-Item $Global:currentExecutingPath).Parent.Parent.FullName) "$("RTS_Config\$model\" + $model + "_" + $Global:OperationPhase + ".xml")"
...
while (-not (Helpers-CheckCtrlC))
{
    ...
    if ($stateMachine -eq "WaitingRequest" -and $requestFolderMonitoring[2].WaitOne(1000))
    {
        ...
        {
            # gets PO info about this SN
            $OP = GetOpInfo $request.result.serialNumber                  

            if (-not $OP) {
                Helpers-TimedTextOut "Não foi encontrada uma OP para esse NS." -ForegroundColor Red
                $success = $false
            }

            $Global:ConfigurationCsv = $null
            $intCD_MATRIZ = 0
            if (-not $OP.CD_MATRIZ -or -not [Int32]::TryParse($OP.CD_MATRIZ, [ref]$intCD_MATRIZ)) {
                Helpers-TimedTextOut "Código da matriz está vazio na informação da OP." -ForegroundColor Red
                $success = $false
            }
            else
            {
                ...
                $SourceImagesFolder |% `
                {
                    if ($SourceServer -eq $null) {
                        if(Test-Path "$_\$($intCD_MATRIZ)\Configuracao.csv" -ErrorAction SilentlyContinue -WarningAction SilentlyContinue) {
                            $SourceServer = $_
                        }
                    }
                ...
                # synchronize the configuration csv file to get the image name
                robocopy.exe (Join-Path $SourceServer $intCD_MATRIZ) $currentExecutingPath Configuracao.csv /E /R:20 /FFT | out-null
                $robocopyExitCode = $LASTEXITCODE
                # http://ss64.com/nt/robocopy-exit.html
                if ($robocopyExitCode -gt 7) {
                    Helpers-TimedTextOut "Erro sincronizando $(Join-Path $SourceServer $intCD_MATRIZ)\Configuracao.csv`r`n Robocopy.exe retornou: $robocopyExitCode" -ForegroundColor Red
                    $success = $false
                }

                # Load the configuration file got from server
                $Global:ConfigurationCsv = Import-Csv -Path (Join-Path $currentExecutingPath "Configuracao.csv") -Delimiter ";"
                if (-not $Global:ConfigurationCsv ) {
                    Helpers-TimedTextOut "Erro ao abrir arquivo CSV local para obter versão da imagem." -ForegroundColor Red
                    $success = $false
                }
                elseif (-not ($Global:ConfigurationCsv | get-member "SoftwareVersion") -or -not $Global:ConfigurationCsv.SoftwareVersion) {
                    Helpers-TimedTextOut "Arquivo CSV não contém coluna SoftwareVersion ou seu valor é vazio." -ForegroundColor Red
                    $success = $false
                }
            }
        }

        if (-not (&$Global:ResponseGenerationFunction $request $success)) {
            MessageBox -Text "FALHA" -ErrorHappened -Title $MsgBoxTitle
        }
        else 
        {
 ...
 if ($enableTEEKey -eq $true -and $stateMachine -eq "WaitingResult" -and $TeeFolderMonitoring[2].WaitOne(1000))
    {
        Helpers-TimedTextOutReset
